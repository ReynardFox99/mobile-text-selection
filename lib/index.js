...
import { getEventPath, getSingleRect, getNextRectByCurrentNode, isSameLine, isPreviousLine, isAfterLine } from "./utils";
  ...
  moveCursor = (element, x, y) => {
    ...
    if (this.movingCursor === this.cursor.start) {
      ...
      if (
        ...
        (isSameLine(endPosition.y, currentY, currentHeight) && currentX < endPosition.x)
      ) {
        // 不需要交换
      ...
      if (
        ...
        (isSameLine(startPosition.y, currentY, currentHeight) && currentX > startPosition.x)
      ) {
        // 不需要交换
        this.textNode.end = new TextNode(node, index);
      } else {
