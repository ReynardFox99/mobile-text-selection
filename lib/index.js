...
export default class TextSelect {
  constructor({
    ...
    menuBackgroundColor,
    menuColor,
    tokenizer = () => {}, // function (sentence) : [index,length]
    onFinish = () => {},
    onStart = () => {},
    onCancel = () => {},
  }) {
  ...
  init = () => {
    ...
    this.magnifier = new Magnifier({
      ...
      backgroundColor: this.magnifierBackgroundColor,
  ...
  destroy = () => {
    ...
    const tools = this.container.getElementsByClassName(
      "mobile-text-selection-tools"
    );
    for (let i = tools.length - 1; i >= 0; i--) {
      tools[i].remove();
    }
    this.container._textSelection = null;
  };
  handleTouchStart = (e) => {
    const event = e;
    // 触摸位置
    ...
    // 是否正在触摸menu
    const touchMenu = getEventPath(event).indexOf(this.menu.element) > -1;
    // 是否正在触摸左右cursor
    const touchStartCursor =
      getEventPath(event).indexOf(this.cursor.start.element) > -1;
    const touchEndCursor =
      getEventPath(event).indexOf(this.cursor.end.element) > -1;

    if (touchMenu) {
      // 正在触摸menu menu中不管是不是长按
      this.touchingMenuKey = this.menu.getSelectMenuKey(getEventPath(event));
      event.stopPropagation();
    } else {
      this.menu.hide(); // 没在触摸menu 则隐藏menu
  ...
  handleTouchMove = (e) => {
    ...
    if (this.selectStatus === "selecting") {
      ...
      if (this.movingCursor) {
        ...
        if (target && target instanceof HTMLIFrameElement) {
          target = target.contentWindow.document.elementFromPoint(
            mouseX,
            mouseY
          );
        }
    ...
    } else if (this.touchingMenuKey) {
      const movedMenuKey = this.menu.getSelectMenuKey(getEventPath(event));
      if (!movedMenuKey || movedMenuKey !== this.touchingMenuKey) {
  ...
  handleTouchEnd = (e) => {
    ...
    this.magnifier.hide();
    const touchPoint = event.changedTouches[0];
    if (this.touchingMenuKey) {
      // 如果是点击菜单 则需要进行菜单的回调
      const text = this.getText();
      this.menu.clickMenuItem(this.touchingMenuKey, { text });
      this.touchingMenuKey = null;
      // TODO: 之后考虑下需不需让用户自己隐藏菜单
      this.blur();
    } else if (
      ...
      if (this.rects.elements.length && text) {
        // 选择完成后显示菜单
        this.menu.position = {
          x: (this.cursor.start.position.x + this.cursor.end.position.x) / 2, // 按选中文字的中间计算 即centerX
          y: this.cursor.start.position.y, // 按选中文字最上边计算 即topY
        };
        this.menu.show();
        // 选择完成后的回调
        this.onFinish({
          text,
          startX: this.cursor.start.position.x, // 第一个字的左上角 相对于container的坐标
          startY: this.cursor.start.position.y,
          endX: this.cursor.end.position.x, // 最后一个字的右上角 相对于container的坐标
  ...
  moveCursor = (element, x, y) => {
    ...
    if (this.movingCursor === this.cursor.start) {
      ...
      if (
        isPreviousLine(endPosition.y, currentY, currentHeight) ||
        (isSameLine(endPosition.y, currentY, currentHeight) &&
      ...
      if (
        isAfterLine(startPosition.y, currentY, currentHeight) ||
        (isSameLine(startPosition.y, currentY, currentHeight) &&
  ...
  handleLongTap = (element, x, y) => {
    ...
    const [cutIndex, cutLength] = (this.tokenizer &&
      this.tokenizer(node, index)) || [-1, 0];
    if (
      typeof cutIndex === "number" &&
      typeof cutLength === "number" &&
      cutIndex > -1 &&
      cutLength > 1
    ) {
      // 有分词
      this.textNode.start = new TextNode(node, cutIndex);
      this.textNode.end = new TextNode(node, cutIndex + cutLength);
      // 计算阴影
      const rects = this.rects.getSelectRects(
        this.textNode.start.node,
        this.textNode.end.node,
      ...
      if (rects.length) {
        ...
        this.cursor.start.position = {
          x: rects[0].x - this.screenRelativeOffset.x,
          y: rects[0].y - this.screenRelativeOffset.y,
        };
        this.cursor.end.position = {
          x:
            rects[rects.length - 1].x +
            rects[rects.length - 1].width -
            this.screenRelativeOffset.x,
          y: rects[rects.length - 1].y - this.screenRelativeOffset.y,
        };
      ...
      // 没有分词
      this.textNode.start = new TextNode(node, index); // index 表示第几个字 (是下标+1的正数)
